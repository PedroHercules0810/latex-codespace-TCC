...
O pôquer possui várias modalidades, sendo algumas como o pôquer \textit{Head's Up}, que consiste em um contra um direto, sendo a modalidade mais famosa a \textit{Texas Hold'Em}, como falado por \citeonline{barbosa2005confiabilidade}, o pôquer é um jogo composto por muitos fatores, sendo os mais importantes para entende-lo: a hierarquia das cartas, a configuraçãoda modalidade, as combinações possíveis, as ações que o jogador pode tomar e a cronologia do jogo.
...
\subsection{As combinações possíveis}
...
\begin{itemize}
    \item[\textit{Straight Flush}]
          É a maior combinação do pôquer, que consiste em cinco cartas em sequência, sendo todas elas do mesmo naipe, dentro dela se encontra o maior jogo possível do pôquer, que é o \textit{Royal Straight Flush} que consiste na sequencia das maiores cartas, do dez até o ás, do mesmo naipe, que é a mostrada no exemplo abaixo:
          \tenc \Jc \Qc \Kc \Ac
    \item[]
    \item[]
\end{itemize}
...
O Q-Learning é um algoritmo de reforço que consiste em dar \textit{Q-Values}(valores de qualidade) para uma determinada ação da rede, considerando assim se vale ou não a pena tomar aquela decisão. O Q-Learning trabalha usando a Q-Table, que consiste em uma tabela que suas linhas são todos os estados possíveis do jogo, enquanto as colunas são todas as ações possíveis, a rede verifica o estado e confere qual ação tem o maior Q-Value e executa a devida ação.
