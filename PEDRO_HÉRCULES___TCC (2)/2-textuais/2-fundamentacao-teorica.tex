...
Também é possível tomar a quantidade de mãos possíveis no \textit{pre-flop}, tendo em vista que são apenas duas cartas, podemos descrever como:

$$\binom{52}{2} = \underbrace{1326}_\text{Combinações de cartas na mão do agente}$$

O que gera 1326 possibilidades de combinações de cartas para cada jogador no \textit{pre-flop}, enquanto as combinações possíveis de cartas comunitárias na mesa, cuja são 5 cartas, pode ser descrito como:

$$\binom{52}{5} = \underbrace{2598960}_\text{combinações de cartas comunitárias possíveis}$$

Além disso, existem mais variáveis que devem ser levadas em consideração pelo agente no pôquer, não apenas as próprias cartas, é necessário calcular quais as possíveis cartas dos outros agentes, as possíveis cartas a virem nas cartas comunitárias, a quantidade de fichas, tanto do próprio agente quanto a dos rivais, possibilidade de jogos serem formados, entre outros, tornando assim um espaço extremamente amplo de probabilidades.


\section{Padrão \textit{MVC}}
...
\begin{itemize}
      ...
      \item \textbf{\textit{Controller} (Controlador)}: Atua como intermediário, processando entradas do usuário (ex.: requisições \gls{HTTP} POST para ações como ``apostar'' ou ``\textit{fold}'') e orquestrando interações entre \textit{Model} e \textit{View}. No pôquer \textit{AI}, o \textit{Controller} gerencia fluxos como inicialização de rodadas, chamadas à rede neural para decisões e execução de simulações \textit{Monte Carlo}, direcionando atualizações no \textit{Model} e renderizando a \textit{View} apropriada.
...
O termo \gls{IA} pode ser considerado um grande universo de técnicas, sendo o termo \gls{IA} o mais amplo deles, emglobando vários outros aspectos, a ferramenta de \gls{IA}  que será abordado é o aprendizado por reforço profundo, uma tecnica de \textit{Machine Learning}, que consiste em um sistema computacional aprender a executar uma ação, ou conjunto de ações, como é citado em \citeonline{russell2010artificial} pode ser entendido como uma criança aprendendo a brincar, ela faz movimentos aleatórios até entender quais movimentos se enquadram melhor para a brincadeira.
Para isso tomamos o modelo de aprendizado, já que o mesmo se enquadra melhor para a criação de uma agente capaz de jogar o \textit{Texas Hold'Em}, dito isto, o agente pode ser baseado em três tipos de aprendizado de máquina, como destacado em \citeonline{russell2010artificial}, sendo alguns deles os seguintes:
\begin{itemize}
...
\begin{itemize}
      ...
      \item \textbf{A ação:} É definida como as ações que o agente pode tomar para obter o melhor resultado possivel, neste caso temos as seguintes ações: \textit{Fold}, \textit{Call} e \textit{Raise};
...
Em muitos casos, essa integral não admite forma fechada ou é impraticável de calcular diretamente, especialmente em espaços de alta dimensão. O método de \textit{Monte Carlo} substitui então essa integral pela média amostral
...
Tal fórmula calcula o \textit{Q-Value}, cada termo significa o seguinte:
\begin{itemize}
