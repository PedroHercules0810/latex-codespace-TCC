...
\section{O jogo pôquer \textit{Texas Hold'Em}}

O pôquer é um dos jogos mais populares da história, como dito por \citeonline{barbosa2005confiabilidade}, possuindo várias modalidades, sendo algumas como o pôquer \textit{Head's Up}, que consiste em um contra um direto, sendo a modalidade mais famosa a \textit{Texas Hold'Em}, o pôquer é um jogo composto por muitos fatores, sendo os mais importantes para entende-lo os seguintes:
...
\begin{itemize}
      \item \textit{Straight Flush}:
            É a maior combinação do pôquer, que consiste em cinco cartas em sequência, sendo todas elas do mesmo naipe. Sendo em caso de empate, se sobressai o jogador que tiver a maior sequência, que pode ser dita como a que termina com a carta de maior valor. Dentro dela se encontra o maior jogo possível do pôquer, que é o \textit{Royal Straight Flush} que consiste na sequencia das maiores cartas, do dez até o ás, do mesmo naipe, que é a mostrada no exemplo abaixo:

            $$ \tenc \Jc \Qc \Kc \Ac $$
      \item Quadra:
            A quadra são quatro cartas de mesmo valor. O desempate é feito pela maior quadra. A quadra pode ser entendida da seguinte forma:
      ...
      \item \textit{Straight}:
            É a sequência de cinco cartas sem importar o naipe das cartas que a compõem. o desempate é feito pela maior sequência, segue um exemplo de \textit{straight}:

            $$ \Ac \twoh \tres \fourd \fivec$$
      \item Trinca:
            É uma combinação formada por três cartas de mesmo valor. O desempate é feito pela maior trinca. Um exemplo de trinca pode ser visto a seguir:
      ...
      \item Carta alta:
            Caso nenhum dos jogadores tenha formado nenhuma combinação, o vencedor é aquele que tiver a maior carta em sua mão. O desempate é feito comparando a maior carta dos jogadores e se for a mesma, a próxima verificada é a segunda maior, assim por diante até a quinta carta de cada um. Segue um exemplo de jogo de carta alta:

            $$ \Ac \twoc \fourd \sevs \Kh$$
\end{itemize}

É válido elucidar que as sequências no pôquer tem um limite, sempre começando no ás, quando se segue do ás até o cinco, sendo a menor sequência possível, e sempre terminam no ás. Sendo a maior sequência possível a que vai do dez até o ás, sendo assim as sequencias não são um ciclo, são uma linha.

\subsection{O desempate}
Já foi citado o desempate específico de cada combinação acima, entretanto, em raros casos, quando dois jogadores possuem a mesma combinação, é necessário aplicar outro metodo de desempate, que é conhecido como \textit{kicker}. Considere o seguinte cenário de empate entre dois jogadores:
...
\begin{itemize}
      ...
      \item \textit{Check}:
            Quando o jogador não quer apostar, ou quando não há apostas para serem cobertas, o jogador pode dar \textit{check}, que funciona como uma passada de vez sem penalidades;

      \item \textit{Raise}:
            O jogador pode dar um \textit{raise} quando quer aumentar uma aposta que outro jogador fez, sendo que um \textit{raise} deve sempre ser o no míimo o dobro da aposta que o mesmo quer aumentar;
      ...
      \item \textit{All-in}:
            É quando o jogador aposta todas as suas fichas disponíveis. O jogador pode optar pelo \textit{all-in} quando tem uma mão muito forte, como blefe, ou quando não tem a quantidade necessárias de fichas para cobrir a aposta, mas ainda deseja continuar na rodada.
\end{itemize}

\subsection{Outros termos do pôquer}

Há ainda alguns termos que devem ser explicados, sendo eles os seguintes:
      ...
      \item \textit{Blind}:
            ...
            \begin{itemize}
                  \item \textit{Small blind}:
                        É metade da aposta mínima paga pelo primeiro jogador da rodada na sequência;
...
\section{Estatística e e o espaço de estados do \textit{Texas Hold'Em}}

De acordo com \citeonline{chen2006mathematics}, grande parte das decisões tomadas em uma partida de pôquer, vêm de condições que ainda não foram determinadas, ou seja, de informações que o agente não possui.
É utilizado o exemplo da probabilidade de ser retirado aleatoriamente uma carta especifica do baralho, sendo que o mesmo possui 52 cartas, há uma chance $\frac{1}{52}$ de uma carta específica ser retirada de forma aleatória.
Também é possível tomar a quantidade de mãos possíveis antes do \textit{pre-flop}, tendo em vista que são apenas duas cartas, podemos descrever como:

$$\binom{52}{2} = \underbrace{1326}_\text{Combinações de cartas na mão do jogador}.$$

O que gera 1326 possibilidades de combinações de cartas para cada jogador antes do \textit{pre-flop}, enquanto as combinações possíveis de cartas comunitárias na mesa, cuja são 5 cartas, tomando que cada jogador recebe duas cartas antes das comunitárias serem distribuídas, pode ser descrito como:

$$\binom{52 - 2*n}{5} = \text{combinações de cartas comunitárias possíveis}.$$

Sendo n a quantidade de jogadores na mesa. Além disso, existem mais variáveis que devem ser levadas em consideração pelo agente no pôquer, não apenas as próprias cartas, mas também é necessário calcular quais as possíveis cartas dos outros agentes, as possíveis cartas a virem nas cartas comunitárias, a quantidade de fichas, tanto do próprio agente quanto a dos rivais, possibilidade de jogos serem formados, entre outros, tornando assim um espaço extremamente amplo de probabilidades.


\section{Padrão \textit{MVC - Model View Control}}
...
A arquitetura da \gls{API} é concebida segundo o modelo \gls{MVC}, um padrão arquitetural clássico de engenharia de software que promove a separação clara de responsabilidades, modularidade, coesão e baixo acoplamento entre componentes. Originado na década de 70 por \citeonline{reenskaug1979}, o \gls{MVC} foi formalizado como um padrão de \textit{design} em \citeonline{gamma1994}, onde é descrito como uma estrutura tripartite para organizar código em aplicações interativas. Esse modelo divide a lógica da aplicação em três camadas interconectadas, mas independentes:

\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.8\linewidth]{lib/Arquitetura MVC.png}
      \caption{Arquitetura MVC - Fonte: Autoria própria}
      \label{fig:MVC}
\end{figure}
...
O termo \gls{IA} pode ser considerado um grande universo de técnicas, sendo o termo \gls{IA} o mais amplo deles, emglobando vários outros aspectos. A ferramenta de \gls{IA}  que será abordado é o aprendizado por reforço profundo, uma técnica de \textit{Machine Learning}, que consiste em um sistema computacional aprender a executar uma ação, ou conjunto de ações. Como é citado em \citeonline{russell2010artificial} pode ser entendido como uma criança aprendendo a brincar, ela faz movimentos aleatórios até entender quais movimentos se enquadram melhor para a brincadeira.
Para isso tomamos o modelo de aprendizado, já que o mesmo se enquadra melhor para a criação de um agente capaz de jogar o \textit{Texas Hold'Em}. Dito isto, o agente pode ser baseado em três tipos de aprendizado de máquina, como destacado em \citeonline{russell2010artificial}, sendo alguns deles os seguintes:
\begin{itemize}
      \item Supervisionado: O agente observa padrões e/ou entradas e saídas que são disponibilizadas para que o mesmo aprenda da melhor maneira como usar aqueles dados para executar suas ações;

      \item Não-supervisionado: O agente trabalha sem necessariamente receber dados para complementar o modelo, como por exemplo um agente de tráfego de táxi, usando um modelo de aprendizado não supervisionado, ele pode usar diretrizes que o mesmo ``percebeu'' para ditar ``Dias com tráfego bom'' ou ``Dias com tráfego ruim'';

      \item Aprendizado por reforço: O agente aprende por uma série de recompensas ou penalidades. Como o agente utilizado neste trabalho é baseado em um aprendizado de máquina por reforço, a maneira do próprio receber as recompensas e punições é feita por conta das fichas do pôquer, sendo ganhar fichas uma recompensa e perder uma penalidade.
...
\begin{itemize}
      \item O agente: É a inteligência artificial propriamente dita, o agente toma as decisões baseadas no ambiente em que o mesmo está enquadrado;

      \item O ambiente: Se trata de onde o agente vai trabalhar. No caso deste trabalho, se referencia à mesa de pôquer \textit{Texas Hold'Em}, seguindo as regras estabelecidas do jogo;

      \item O estado: Pode ser dito como os elementos que estão à disposição do agente, e que neste caso, são váriaveis dependendo do momento do ambiente. Neste caso as fichas e as cartas tomam o papel de estado;

      \item A ação: É definida como as ações que o agente pode tomar para obter o melhor resultado possível. Neste caso temos as seguintes ações: \textit{Fold}, \textit{Call} e \textit{Raise};

      \item A recompensa: Como o aprendizado utilizado é o aprendizado por reforço, o sistema de recompensa é essencial para moldar o agente da melhor maneira possível. No caso deste trabalho o sistema de recompensa se baseia no lucro ou prejuízo de fichas, sendo ganhar fichas uma recompensa, enquanto perder fichas, ou deixar de ganhar, gera uma penalidade.
\end{itemize}

\section{Método de \textit{Monte Carlo} aplicado a uma rede neural jogadora de pôquer}

O método de \textit{Monte Carlo} é uma classe de técnicas numéricas baseadas em amostragem aleatória para estimar quantidades de interesse em problemas cuja solução analítica é inviável ou extremamente custosa. Historicamente, o método foi formalizado na década de 1940 no contexto do Projeto Manhattan em Los Alamos, quando Stanislaw Ulam e John von Neumann passaram a utilizar amostragens aleatórias para resolver problemas complexos de transporte de nêutrons e física nuclear, em substituição a cálculos combinatórios e integrais de alta dimensionalidade \citeonline{leonelli2021montecarlo}. O nome ``\textit{Monte Carlo}'' foi sugerido por Nicholas Metropolis em referência ao famoso cassino de Mônaco, ressaltando a analogia com experimentos de sorteio e jogos de azar.

De forma geral, o objetivo do método de \textit{Monte Carlo} é aproximar uma quantidade esperada, como uma integral ou esperança matemática, por meio da média de amostras geradas aleatoriamente. Dado um vetor aleatório \(X\) com distribuição \(p(x)\) e uma função de interesse $ f(x) $, deseja-se estimar a esperança

$$\mu = \mathbb{E}[f(X)] = \int_{-\infty}^{\infty} f(x)\, p(x)\, dx.$$

Em muitos casos, essa integral não admite forma fechada ou é impraticável de calcular diretamente, especialmente em integrais múltiplas. O método de \textit{Monte Carlo} substitui então essa integral pela média amostral
$$\hat{\mu}_N = \frac{1}{N} \sum_{i=1}^{N} f(X_i),$$
em que \(X_1, X_2, \dots, X_N\) são amostras independentes e identicamente distribuídas segundo \(p(x)\). Pelo Teorema da Lei dos Grandes Números, \(\hat{\mu}_N\) converge quase certamente para \(\mu\) quando \(N \to \infty\), e o Erro Padrão da estimativa diminui aproximadamente com a ordem de \(1/\sqrt{N}\) \citeonline{rubinstein2008simulation}. Essa propriedade torna o método de Monte Carlo particularmente útil para problemas de alta complexidade, onde métodos determinísticos teriam custo proibitivo.

Do ponto de vista prático, um algoritmo básico de Monte Carlo para estimar \(\mu\) pode ser descrito em três passos \citeonline{rubinstein2008simulation}:
\begin{enumerate}
...
\end{enumerate}
\citeonline{rubinstein2008simulation} destacam que, embora o método seja conceitualmente simples, boa parte da pesquisa em \textit{Monte Carlo} concentra-se em aprimorar a geração de amostras e reduzir a variância das estimativas (por exemplo, via amostragem estratificada, variáveis antitéticas ou amostragem por importância) \citeonline{rubinstein2008simulation}.

\subsection{Aplicação do método de \textit{Monte Carlo} em jogos e pôquer}

Jogos de cartas como o pôquer são domínios clássicos para aplicação de \textit{Monte Carlo}, pois envolvem incerteza combinatória (cartas ocultas, cartas futuras), múltiplos agentes e grandes espaços de estados. Em pôquer \textit{Texas Hold'em}, por exemplo, um jogador conhece apenas suas cartas privadas e as cartas comunitárias já reveladas, mas desconhece as cartas dos oponentes e as cartas que ainda serão abertas. Isso torna difícil calcular exatamente a probabilidade de vitória ou o valor esperado de uma ação (pagar, aumentar, desistir) em tempo real.

O método de \textit{Monte Carlo} é empregado para contornar essa dificuldade por meio de simulações do futuro da mão. A ideia básica é:
\begin{enumerate}
...
Diversos trabalhos em pôquer computacional adotam variações desse esquema, tanto para avaliar a força de uma mão quanto para apoiar decisões estratégicas \citeonline{kleij2010thesis,vdBroeck2009mcts,heuristic2015poker}. Por exemplo, Van der Kleij construiu um jogador de pôquer computacional que utiliza busca em árvore por \textit{Monte Carlo}, combinada com modelagem de oponentes para estimar valores esperados de ações em diferentes estágios do jogo \citeonline{kleij2010thesis}. Em outro estudo, Van den Broeck e colaboradores aplicam \gls{MCTS} em pôquer no-limit, utilizando simulações para explorar o espaço de decisões possíveis e discretizar tamanhos de aposta, reduzindo o custo computacional sem perder competitividade \citeonline{vdBroeck2009mcts}. Esses trabalhos demonstram que, mesmo em jogos de informação imperfeita, o uso sistemático de simulações aleatórias permite aproximar o valor das ações de forma eficaz.

Em contextos mais heurísticos, \textit{Monte Carlo} também é amplamente utilizado para validar estratégias ou heurísticas de decisão em pôquer. Por exemplo, estudos utilizam simulações de centenas de milhares ou milhões de mãos para estimar a probabilidade de ocorrência de determinados tipos de mãos ou para verificar o desempenho de regras fixas de jogo ao longo do tempo \citeonline{heuristic2015poker}. Essa mesma lógica de simulação extensiva é diretamente aproveitada em arquiteturas de inteligência artificial, nas quais a estimativa robusta de probabilidades e valores esperados é um insumo fundamental para o treinamento de modelos.

\subsection{Integração do método de \textit{Monte Carlo} com redes neurais para pôquer}

No contexto desta pesquisa, o método de \textit{Monte Carlo} é empregado como um mecanismo de estimação estatística que gera alvos ou sinais de treinamento para uma rede neural projetada para jogar pôquer. A integração entre \textit{Monte Carlo} e aprendizado de máquina segue a linha de trabalhos em aprendizado por reforço (\textit{Reinforcement Learning}), nos quais estimativas de retorno obtidas a partir de episódios completos, que são as rodadas do pôquer, são utilizadas para atualizar funções de valor e políticas, sendo elas as estratégias que o agente aprende.
...
\begin{itemize}
      \item Geração de rótulos a partir de simulações
      \item Métodos de \textit{Monte Carlo} em aprendizado por reforço
\end{itemize}

\subsubsection{Geração de rótulos a partir de simulações}

Uma abordagem direta consiste em usar simulações de \textit{Monte Carlo} para rotular estados com estimativas de valor ou probabilidade de vitória. O procedimento típico é:
\begin{enumerate}
...
\end{enumerate}
Essa estratégia transforma o problema em um cenário de aprendizado supervisionado onde \textit{Monte Carlo} fornece um especialista para a rede, à semelhança do que \citeonline{sutton2018rl} discutem para métodos de \textit{Monte Carlo} em aprendizado por reforço, nos quais o retorno total observado em um episódio serve como alvo para a função de valor.

\subsubsection{Métodos de \textit{Monte Carlo} em aprendizado por reforço}

Outra forma de integração é empregar diretamente métodos de \textit{Monte Carlo} em aprendizagem por reforço, em que a própria interação da rede com um ambiente simulador de pôquer gera episódios de jogo. Nesse cenário:
\begin{itemize}
      ...
      \item Esses retornos \(G_t\) são utilizados para atualizar os parâmetros da rede, seja como alvos para uma função de valor \(V(s)\), seja como sinal de reforço para ajustar uma política parametrizada \(\pi_\theta(a|s)\), em que ``a'' representa a ação que o agente toma.
\end{itemize}
\citeonline{sutton2018rl} mostram que tais métodos de \textit{Monte Carlo} permitem aprender diretamente a partir da experiência, sem a necessidade de um modelo completo das dinâmicas do ambiente, o que é particularmente atraente em domínios complexos como o pôquer, em que a modelagem exata dos oponentes e das distribuições de cartas é difícil. Na prática, pode-se combinar simulações de \textit{Monte Carlo} com técnicas como busca em árvore \gls{MCTS} ou amostragem de políticas para explorar as consequências de diferentes ações antes de atualizá-las na rede neural.

\subsection{Aspectos práticos e considerações metodológicas}

A aplicação do método de \textit{Monte Carlo} em pôquer associado a redes neurais envolve algumas decisões metodológicas importantes:
\begin{itemize}
      \item Número de simulações (\(N\)): Há um compromisso entre custo computacional e precisão estatística. Valores de \(N\) muito baixos resultam em estimativas ruidosas, enquanto valores muito altos aumentam o tempo de treinamento. Em geral, escolhe-se \(N\) de forma a garantir erros padrão aceitáveis, sendo esses erros aceitáveis uma perda ``mínima'' de fichas, para as estimativas de valor, muitas vezes com base em experimentos preliminares como dito por \citeonline{rubinstein2008simulation}.
      \item Modelagem de oponentes: Em jogos de informação imperfeita, as simulações devem refletir crenças realistas sobre o comportamento dos oponentes. Trabalhos em pôquer computacional utilizam modelos paramétricos de oponentes, aprendidos a partir de dados, para amostrar ações e cartas de forma consistente com estilos de jogo observados por \citeonline{kleij2010thesis,vdBroeck2009mcts}.
      \item Representação de estados: A qualidade das estimativas de \textit{Monte Carlo} depende também da expressividade da representação utilizada na rede neural. Entradas que resumem adequadamente a informação relevante (por exemplo, tamanhos relativos de pilha, posição, textura do bordo e histórico de ações) favorecem o aprendizado de padrões estáveis a partir dos retornos simulados.
      \item Redução de variância: Técnicas clássicas de redução de variância podem ser adaptadas ao contexto de pôquer, por exemplo, estratificando simulações por tipo de mão inicial ou por determinadas configurações de bordo, de modo a obter estimativas mais precisas sem necessariamente aumentar \(N\) como notado por \citeonline{rubinstein2008simulation}.
\end{itemize}

Dessa forma, o método de \textit{Monte Carlo} funciona como um elemento estruturante, fornecendo a base estatística para a estimativa de probabilidade de vitória, valor esperado de ações e retornos de aprendizado por reforço.

\section{Redes neurais artificiais e \textit{Q-Learning}}
...
O \textit{Q-Learning} é um algoritmo de aprendizado por reforço que associa valores de qualidade, \textit{Q-Values}, às ações disponiveis em determinado estado, de modo a estimar a melhor decisão. O \textit{Q-Learning} trabalha usando a \textit{Q-Table}, que consiste em uma tabela que suas linhas são todos os estados possíveis do jogo, enquanto as colunas são todas as ações possíveis, a rede verifica o estado e confere qual ação tem o maior \textit{Q-Value} e executa a devida ação.

Para calcular o valor ideal do \textit{Q-Value}, como citado por \citeonline{mnih2015human}, é utilizada a fórmula de Bellman, cuja é a seguinte:
$$Q_{i+1}(s, a) = r + \gamma \max Q_i(s', a')$$
Tal fórmula calcula o \textit{Q-Value} e cada termo significa o seguinte:
\begin{itemize}
      ...
      \item $\max Q_i(s', a')$: Esta função representa o maior \textit{Q-Value} possível, tomando o estado futuro (\textit{s'}) tomando a melhor ação possível (\textit{a'}).
\end{itemize}

Entretanto, um algoritmo de \textit{Q-Learning} tradicional funciona perfeitamente com jogos mais simples e com menos ações e estados a serem verificados. Jogos como jogo da velha ou labirintos são exemplos simples para se utilizar o \textit{Q-Learning} tradicional, porém o \textit{Texas Hold'Em}, por conta da quantidade de combinações de cartas, tamanho do pote, quantidade de fichas do jogador e ações a serem tomadas, criaria uma tabela que beiraria a infinidade de estados que poderiam ser tomados, tornando ineficaz e praticamente impossível a criação de uma \textit{Q-Table} para um jogo de pôquer, tendo em vista que nenhum computador moderno teria capacidade de memória para alocar uma tabela tão extensa.


\section{\textit{Deep Q-Network}}
O \textit{Deep Q-Network} é uma variação do algoritmo de Q-Leaning padrão. O mesmo é usado quando a quantidade de estados beira o infinito, como falado por \citeonline{mnih2015human}, já que o mesmo usa de uma rede neural para fazer a predição do \textit{Q-Value}.

Como citado anteriormente, esta abordagem não é eficiente já que é calculada sem nenhuma generalização, sendo assim, o \textit{Deep Q-Network} foi criado para que fosse possível que a rede neural fizesse a predição sem necessidade de criar uma tabela grande demais até mesmo para ser alocada na memória, assim podendo usar uma equação para se calcular o valor aproximado do \textit{Q-Value}, usando a equação:
...
O \gls{DQN} foi desenvolvido por \citeonline{mnih2015human} para ensinar uma Inteligencia artificial para jogar Atari, tornando-se um marco histórico para o desenvolvimento de Redes Neurais utilizando o \textit{Q-Learning}.

\section{Exploração vs. Explotação (\textit{Epsilon-Greedy})}

O agente necessita aferir a melhor jogada possível do \textit{Texas Hold'Em}, para isso ele deve explorar suas opções de jogada. Inicialmente o mesmo não tem conhecimento para determinar a melhor jogada possível, logo o agente deve escolher aleatoriamente sua jogada, antes de conseguir explotar as melhores jogadas possíveis.

Para isso, foi usada a tática \textit{$\epsilon$-Greedy}(Epsilon-Guloso), como descrita em \citeonline{sutton2018rl}, o \textit{$\epsilon$-Greedy} consiste em uma tática em que o coeficiente de exploração $\epsilon$ determina a aleatoriedade das ações. No caso deste trabalho, o mesmo é iniciado em 1.0, determinando exploração total, ou seja, uma ação completamente aleatória, que vai decaindo ao decorrer das jogadas, chegando próximo de 0.0, assim maximizando a explotação. Sendo assim, reduzindo a complexidade, já que não é necessário encontrar a melhor solução possível, necessitando apenas encontrar uma que atenda o interesse imediato.

\section{Replay de Experiência (\textit{Experience Replay})}

Pela extensa quantidade de estados que o \textit{Texas Hold'Em} pode tomar, alguns estados se tornam semelhantes entre si, o que pode ocasionar um ótimo local para a Rede Neural, isso é denotado por \citeonline{mnih2015human}, quando é citado que casos semelhantes ``viciam'' nas ações, tornando o agente enviesado.

Para contornar este problema foi utilizado o \textit{Experience Replay}(Replay de experiência), também citado em \citeonline{mnih2015human}. O \textit{Experience Replay} consiste em criar lotes de memórias aleatórias do passado para serem usados antes do agente decidir a jogada, servindo como parte do treino, e assim evitando enviesamentos do agente, deixando o mesmo mais robusto e mais assertivo com a proposta do jogo.