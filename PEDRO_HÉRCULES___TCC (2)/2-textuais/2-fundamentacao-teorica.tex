...
A arquitetura da \gls{API} é concebida segundo o modelo \gls{MVC}, um padrão arquitetural clássico de engenharia de software que promove a separação clara de responsabilidades, modularidade, coesão e baixo acoplamento entre componentes. Originado na década de 70 por \citeonline{reenskaug1979}, o \gls{MVC} foi formalizado como um padrão de \textit{design} no livro ``\textit{Design Patterns: Elements of Reusable Object-Oriented Software}'' \citeonline{gamma1994}, onde é descrito como uma estrutura tripartite para organizar código em aplicações interativas. Esse modelo divide a lógica da aplicação em três camadas interconectadas, mas independentes:

\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.8\linewidth]{lib/Arquitetura MVC.png}
      \caption{Arquitetura MVC: Autoria própria}
      \label{fig:MVC}
\end{figure}
...
\begin{itemize}
      ...
      \item \textbf{O ambiente:} Se trata de onde o agente vai trabalhar. No caso deste trabalho, se referencia à mesa de pôquer \textit{Texas Hold'Em}, seguindo as regras estabelecidas do jogo;
      ...
      \item \textbf{A ação:} É definida como as ações que o agente pode tomar para obter o melhor resultado possível. Neste caso temos as seguintes ações: \textit{Fold}, \textit{Call} e \textit{Raise};
...
\end{enumerate}
Rubinstein e Kroese destacam que, embora o método seja conceitualmente simples, boa parte da pesquisa em \textit{Monte Carlo} concentra-se em aprimorar a geração de amostras e reduzir a variância das estimativas (por exemplo, via amostragem estratificada, variáveis antitéticas ou amostragem por importância) \citeonline{rubinstein2008simulation}.
