...
Esta simulação demonstra o funcionamento do motor base 15, já que o \textit{Jogador[0]} e o \textit{Jogador[2]} possuíam dois pares, porém o \textit{Jogador[0]} levou a vitória por pontuar mais, já que, seguindo a formula para o motor de desempate o \textit{jogador[0]} pontuou 2.819.809 pontos, enquanto o \textit{jogador[2]} pontuou 2.819.098, uma diferença de 711 pontos, mesmo o \textit{Jogador[2]} possuindo um kicker de Ás, porém os pares do \textit{jogador[0]} eram superiores.

\section{Evolução na taxa de exploração}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{lib/Decaimento Epsilon.jpeg}
    \caption{Decaimento do fator de exploração épsilon ($\epsilon$).}
    \label{fig:decaimentoEpsilon}
\end{figure}

O gráfico demonstra o decaimento do fator de exploração épsilon ($\epsilon$), sendo possível notar que a partir de por volta do jogo 500, a taxa de exploração é mínima, significando que o agente esta jogando com tudo o que aprendeu, ao invés de apenas tomar ações aleatórias.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{lib/Desempenho do agente.jpeg}
